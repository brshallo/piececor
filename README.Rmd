...
For example, say we want to see the p.value's of correlations at each segment for `hp ~ qsec`:
...
The `cor` column here actually represents the statistical test (run separately on each segment) of the null hypothesis that the spearman correlation coefficient is 0.
...
* Output of `weighted_abs_mean_cors()` is not particularly meaningful in a traditional notion of "correlation."
    ...
    * Note that the similar package [nlcor](https://github.com/ProcessMiner/nlcor) just takes the mean of the absolute value of the correlations to get an adjusted total correlation and they do $1 - \prod_{n = 1}^{i}(1 - p_n)$ to get adjusted p values of the correlations. See `nlcor:::NetCor()` [here](https://github.com/ProcessMiner/nlcor/blob/master/R/correlations.R).
